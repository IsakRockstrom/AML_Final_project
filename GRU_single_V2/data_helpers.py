...
def _create_lagged_rollout(X, y, lag, horizon, delta = False):

    X_lag, y_lag = [], []
    baselines = []
    for i in range((len(X) - horizon) // lag):
        i += 1
        X_lag.append(X[lag * i - lag : lag * i])
        y_lag.append(y[lag * i : lag * i + horizon])
        baselines.append(y[lag * i - 1])

    X_lag = np.array(X_lag)
    y_lag = np.array(y_lag)

    if delta:
        y_with_base = np.column_stack([baselines, y_lag])  
        y_lag = np.diff(y_with_base, axis=1)
